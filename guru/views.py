"""Views for the conversational workspace and file downloads."""
...
def workspace_sample_agent(request):
    """Render workspace workspace view and process files uploads/RAG question answers."""
    session_key = request.session.session_key or request.session.create()

    context = {
        "nav_left_label": "Enterprise Contract Analyzer",
        "hero_title": "📄 AI-Powered Contract Review Platform",
        "hero_subtitle": (
        ...
        "show_clause_summary_column": SHOW_CLAUSE_SUMMARY_COLUMN,
        "analysis_status": "Analysis status will be shown here"
    }
    ...
    if request.method == "POST":
        ...
        if action == "remove_file":
            ...
            try:
                ...
                context["sections"] = result.get("sections", [])
                context["classified_clauses"] = result.get("classified_clauses", [])
                context["compliance_results"] = result.get("compliance_results", [])
                context["risk_results"] = result.get("risk_results", [])
                context["mitigation_results"] = result.get("mitigation_results", [])
                ...
                context["analysis_status"] = "Compliance analysis completed"

                print("Sections:", len(context["sections"]))
                print("Classified:", len(context["classified_clauses"]))
                print("Compliance:", len(context["compliance_results"]))
                print("Risk:", len(context["risk_results"]))
                print("Executive Summary:", context.get("executive_summary", {}))

            except Exception as err:
                context["error"] = str(err)
                context["analysis_status"] = "Analysis failed"
    ...
    return render(request, "guru/workspace_sample_agent.html", context)
...
def download_report(request):
    """Download the main Word document analysis report."""
...
def download_table_docx(request):
    """Download individual tables (compliance, risk, mitigation) as Word documents."""
...
def download_report_pdf(request):
    """Download the full contract analysis report formatted as a PDF."""
    session_key = request.GET.get("session_key")
    if not session_key:
        session_key = request.session.session_key or request.session.create() or request.session.session_key
...
def download_table_pdf(request):
    """Download individual tables (compliance, risk, mitigation) as PDFs."""
    session_key = request.GET.get("session_key")
    if not session_key:
        session_key = request.session.session_key or request.session.create() or request.session.session_key
